package tw.skyarrow.ehreader;

public class Constant {
    public static final String BASE_URL = "http://g.e-hentai.org/";
    public static final String BASE_URL_EX = "http://exhentai.org/";

    public static final String API_URL = BASE_URL + "api.php";
    public static final String API_URL_EX = BASE_URL_EX + "api.php";

    public static final String GALLERY_URL = BASE_URL + "g/%d/%s";
    public static final String GALLERY_URL_EX = BASE_URL_EX + "g/%d/%s";

    public static final String PHOTO_URL = BASE_URL + "s/%s/%d-%d";
    public static final String PHOTO_URL_EX = BASE_URL_EX + "s/%s/%d-%d";

    public static final String IMAGE_SEARCH_URL = "http://ul.e-hentai.org/image_lookup.php";
    public static final String IMAGE_SEARCH_URL_EX = "http://ul.exhentai.org/image_lookup.php";

    public static final String AUTHOR_PAGE = "https://twitter.com/tommy351";
    public static final String HOMEPAGE = "https://github.com/tommy351/ehreader-android";

    public static final String AVATAR_SRC = "http://forums.e-hentai.org/uploads/av-";
    public static final String LOGIN_URL = "https://forums.e-hentai.org/index.php?act=Login&CODE=01";
}
